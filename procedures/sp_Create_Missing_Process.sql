	...
	WHILE EXISTS (SELECT TOP 1 ERezeptID FROM #Zuweisung)
		BEGIN
			...
			DECLARE @PZN INT = (SELECT TOP 1 dipzn FROM RW_APOBASE_Rezept WHERE ERezeptID = @CurrentValue AND diPzn NOT IN ((9999175), (9999637), (9999005), (9999057), (2567024), (6461110), (2567001), (2567018), (8000001), (9999643)));
				...
				SELECT 
					ERezeptID AS Id, 
					CONVERT(VARCHAR(10), CONVERT(DATE, CAST(diDate AS CHAR(8))), 105) AS Date, 
					RIGHT('00' + LEFT(RIGHT('000000000' + CAST(diTime AS VARCHAR), 9), 2), 2) + ':' + SUBSTRING(RIGHT('000000000' + CAST(diTime AS VARCHAR), 9), 3, 2) AS Time,
					iRpNr AS Rp, 
					CONVERT(VARCHAR(10), CONVERT(DATE, CAST(@ZuweisungsDatum AS CHAR(8))), 105) AS AssignedDate, 
					RIGHT('00' + LEFT(RIGHT('000000000' + CAST(@Time AS VARCHAR), 9), 2), 2) + ':' + SUBSTRING(RIGHT('000000000' + CAST(@Time AS VARCHAR), 9), 3, 2) AS AssignedTime,
					@VG AS AssignedVg, 
					@RP AS AssignedRp 
				from #ERezeptKopie 
