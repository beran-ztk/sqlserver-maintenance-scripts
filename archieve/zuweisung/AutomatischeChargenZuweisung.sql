	...
	DECLARE @ZuweisungDatum CHAR(8);
	SET @ZuweisungDatum = FORMAT(DATEADD(MONTH, -9, DATEFROMPARTS(YEAR(@CurrentDate), MONTH(@CurrentDate), DAY(@CurrentDate))), 'yyyyMMdd');
	...
	FROM RW_APOBASE_Rezept r
	  ...
	  LEFT JOIN RW_EREZEPT_Dispense d ON r.ERezeptID = d.ERezeptID
	...
	WHILE EXISTS (SELECT TOP 1 ERezeptID FROM #Zuweisung)
		BEGIN
			...
			DECLARE @Charge nvarchar(25) = (SELECT top 1 s.Charge FROM RW_MODUL_SEC_Securpharm s LEFT JOIN RW_MODUL_SEC_VORGANG_SECURPHARM vs ON s.SecurPharmID = vs.SecurpharmID LEFT JOIN RW_APOBASE_Vorgaenge v ON vs.diDate = v.diDate AND vs.iVgNr = v.iVgNr AND vs.diPzn = v.diPzn WHERE v.diPzn = @PZN AND vs.diDate > @ZuweisungDatum AND s.Charge IS NOT NULL ORDER BY DT DESC);
